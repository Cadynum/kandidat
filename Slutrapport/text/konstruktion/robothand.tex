...
\label{fig:servo}
\end{figure}
Dessa servomotorer, se \ref{chp:komponentlista}, används för att de uppfyller kraven på vridmoment med god marginal, se \ref{servodim} för dimensionsberäkningar. Vid en matningsspänning på 6 Volt har servot ett maximalt vridmoment på \unit[1.42]{N} och en högsta rotationshastighet på \unit[6.16]{rad/s} utan belastning. Servona har ett totalt rörelseomfång på 120 grader vilket är standard för hobbyservos. Servona regleras via PWM-signaler och har en intern positionsreglering, detta gör att servomotorerna alltid arbetar för att nå det önskade läget och återgår till detta läge efter en eventuell störning.
