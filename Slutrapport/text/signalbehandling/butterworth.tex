...
\label{sec:butter}
För att filtera signaler används ett \emph{Butterworthfilter} av första ordningen. Filtret är ett lågpassfilter med minimal fasförskjutning för kausala lågpassfilter. Låt $\omega_0$ vara brytfrekvensen i $\unit{rad/s}$ och $G(s)$ överföringsfunktionen i laplace-domän:
\begin{equation}
\label{eq:butter_s}
...
\subsection{Latenstid}
Butterworthfiltret fasförskjutning för olika frekvenser varierar (se figur~\ref{fig:butter_phase}).
För att mäta hur lång fördröjning filtret introducerar för en viss frekvens kan vi normalisera fasförskjutningen i fourier-domänen mot dess frekvens $f$. \\
Låt
\begin{itemize}
    \item $G(s)$ filtrets överföringsfunktion i laplace-domänse, där vi studerar funktionens komplexa frekvens genom att sätta $s=2\i f$. Se ekvation~\eqref{eq:butter_s}.
